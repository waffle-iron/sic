...
public class GUI_AbrirCaja extends JDialog {

    private static final Logger LOGGER = Logger.getLogger(GUI_AbrirCaja.class.getPackage().getName());
    
    public GUI_AbrirCaja(boolean modal) {
        this.setModal(true);
        initComponents();
        this.setModelSpinner();
        ImageIcon iconoVentana = new ImageIcon(GUI_AbrirCaja.class.getResource("/sic/icons/Caja_16x16.png"));
    ...
    private void btn_AbrirCajaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_AbrirCajaActionPerformed
        ...
        try {
            GUI_Caja abrirCaja = new GUI_Caja(RestClient.getRestTemplate()
                    .postForObject("/cajas",
